...
import { requestChangesVendor } from "@/services/vendor";
...
export async function POST(request: Request, { params }: { params: Promise<{ vendorId: string }> }) {
  try {
    ...
    const vendor = await requestChangesVendor(parsed.data.vendorId, user.id, payload.data.note);
    return NextResponse.json({ vendor });
  } catch (e) {
    return fromServiceError(e, { error: "Request changes failed", code: "REQUEST_CHANGES_FAILED" });
  }
}
